...
public:
    ...
    void mute_on(); //command to turn the projector off
    void mute_off(); //command to turn the projector on
    std::string attemptConnection();
    bool getIsTCPClientConnected();
    bool pingIPAddress(std::string _IP);
    void sendPJLinkCommand(std::string command, bool sendOn); //send any PJLink command to the projector
    ...
    void sendCommand(std::string command, bool sendOn); //send any string command to the projector without password authentication
...
private:
	...
	void pjLink_Off();
    void pjLink_mute_on();
    void pjLink_mute_off();
    void pjLink_get_power_status();
	void sanyo_On();
	void sanyo_Off();
	void christie_On();
	void christie_Off();
